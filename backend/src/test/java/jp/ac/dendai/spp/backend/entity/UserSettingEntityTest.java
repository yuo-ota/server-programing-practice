...
public class UserSettingEntityTest {
  ...
  void testPersistAndRetrieveUser() {
    ...
    UserSetting userSetting = new UserSetting(savedUser.getUserId(), "Test User", "testuser", LocalDate.parse("2000-01-01"), false);
    UserSetting savedUserSetting = userSettingRepository.save(userSetting);

    // UUIDが自動生成されていることを確認
    assertThat(savedUserSetting.getUserId()).isNotNull();
    // データベースから取得
    Optional<UserSetting> retrievedUserSettingOpt = userSettingRepository.findById(savedUserSetting.getId());
